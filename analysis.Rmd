---
title: "German Credit Data Analysis with Interactive Model Training"
...
We can take a look, if there are high correlations between the variables. Correlations for numerical variables:
```{r}
cor(cbind(df[int_vars],data.frame(target=as.integer(df$target))))
```

There are no high correlations between the numerical variables and the target variable, and between the numerical variables, the highest correlation is between *duration* and *amount*, which makes sense - the higher the credit amount, the longer time you need to pay it back.


Next, we may look at the categorical variable similarities by performing a categorical dimension reduction [MCA](http://www.sthda.com/english/wiki/multiple-correspondence-analysis-essentials-interpretation-and-application-to-investigate-the-associations-between-categories-of-multiple-qualitative-variables-r-software-and-data-mining), and looking at two of the most meaningful dimensions and variable category angles.
```{r results='hide', warning=FALSE, message=FALSE}
require(FactoMineR)
require(factoextra)
```
```{r}
res.mca <- MCA(df[,c(cat_vars,"target")], graph=F)
fviz_mca_var(res.mca) + theme_bw()
```

From this plot we can observe that *checking*, *history*, *job* and *coapp* have similar angle to *bad* from the center (0,0). However, the percentages for Dim1 and Dim2 are low, which means that the first two dimension explain less than 10 % of the total variability. This indicates that possible dimension reduction on the categorical variables might not make sense.
